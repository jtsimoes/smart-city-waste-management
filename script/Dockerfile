...
COPY --from=0 ../vanetza/vanetza/bin/socktap /usr/local/bin/socktap
COPY --from=0 /root/go/src/dds-vanetza-service/Vanetza_DDS_Spec.xml /Vanetza_DDS_Spec.xml
COPY --from=0 ../vanteza/vanetza/tools/socktap/config.ini /config.ini
COPY --from=0 ../vanetza/vanetza/entrypoint.sh /entrypoint.sh
